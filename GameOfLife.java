...
public class GameOfLife {
	...
	private static void test2(String fileName) {
		int[][] board = read(fileName);
		//System.out.println(count(board, 2, 3));
	...
	public static int count(int[][] board, int i, int j) {
		int sumTopRow = board[i-1][j-1] + board[i-1][j] + board[i-1][j+1];
		
		int sumCurrentRow = board[i][j-1] + board[i][j+1];
		
		int sumBottomRow = board[i+1][j-1] + board[i+1][j] + board[i+1][j+1];
		
		return sumTopRow + sumCurrentRow + sumBottomRow;
	}
